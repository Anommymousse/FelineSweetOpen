...
public class EnemyHugWallMinimal : MonoBehaviour
{
    ...
    bool IsNextMoveIntoBrick()
    {
        ...
        destPosition += (Vector3)(_currentDirection * _speed * Time.deltaTime);
    ...
    void UpdateCellPosition()
    {
        ...
        Vector2 destPosition = Vector3.zero;//  transform.position;
        destPosition += (Vector2)(_currentDirection * _speed * Time.deltaTime);
        _currentCellPos = _nonHiddenRef.WorldToCell(transform.position);
        
        _rigidbody2D.MovePosition(_rigidbody2D.position + destPosition);        
