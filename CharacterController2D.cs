...
public class CharacterController2D : MonoBehaviour
{
	...
	void UpdateGrounded()
	{
		...
		var raycastHit2DLeft = Physics2D.RaycastAll(m_GroundCheckLeft.position, Vector2.down, 0.2f,m_WhatIsGround);
		var raycastHit2DRight =  Physics2D.RaycastAll(m_GroundCheckRight.position, Vector2.down, 0.2f,m_WhatIsGround);
		var raycastHit2DTopRight = Physics2D.RaycastAll( m_CeilingWallCheckRight.position, Vector2.down, 0.2f,m_WhatIsGround);
		var raycastHit2DTopLeft = Physics2D.RaycastAll( m_CeilingWallCheckLeft.position, Vector2.down, 0.2f,m_WhatIsGround);
		

		_righttest = (raycastHit2DRight.Length > 0);
		_lefttest = (raycastHit2DLeft.Length > 0);
		_leftToptest = (raycastHit2DTopLeft.Length > 0);
		_rightToptest = (raycastHit2DTopRight.Length > 0);

		if ((raycastHit2DMiddle.Length > 0)) m_Grounded= true;
		
		//Problems with dash and dropping...
		if ( (raycastHit2DTopRight.Length < 1) & (raycastHit2DRight.Length > 0)) m_Grounded = true;
		if ( (raycastHit2DTopLeft.Length < 1) & (raycastHit2DLeft.Length > 0)) m_Grounded = true;
